%% BAYESIAN OPTIMIZATION (masses and cascade case)

%% Cleanup %%

clear;
close all;
clear obj_masses_panda
clear obj_PID_panda_total_masses
...
project_case = 3;
[parallel_var, sat_var, rampa_var, exploration_PID_iters, exploitation_PID_iters, num_Seed_points_PID, exploration_masses_iters, exploitation_masses_iters, num_Seed_points_masses] = user_requests(project_case);
...
complete_results_masses_total = cell(2, 2);
...
bound_min_m1 =  m1_opt*(1-perc_bound);
bound_max_m1 =  m1_opt*(1+perc_bound);

bound_min_m3 =  m3_opt*(1-perc_bound);
bound_max_m3 =  m3_opt*(1+perc_bound);

bound_min_m5 =  m5_opt*(1-perc_bound);
bound_max_m5 =  m5_opt*(1+perc_bound);

bound_min_m7 =  m7_opt*(1-perc_bound);
bound_max_m7 =  m7_opt*(1+perc_bound);

opt_vars_masses = [opt_vars_masses optimizableVariable('m1', [bound_min_m1, bound_max_m1],'Type','real')];
opt_vars_masses = [opt_vars_masses optimizableVariable('m3', [bound_min_m3, bound_max_m3],'Type','real')];
opt_vars_masses = [opt_vars_masses optimizableVariable('m5', [bound_min_m5, bound_max_m5],'Type','real')];
opt_vars_masses = [opt_vars_masses optimizableVariable('m7', [bound_min_m7, bound_max_m7],'Type','real')];

%% MASSES BAYESIAN OPTIMIZATION

func_masses = @(x_vars)(obj_masses_panda(x_vars,const));
initial_X = []; 
clear objBO; 

%% FIRST PHASE: EXPLORATION MASSES

results_masses_explore = bayesopt(func_masses, opt_vars_masses, ...
    'Verbose', 0, ...
    'AcquisitionFunctionName', ...
    'lower-confidence-bound'	, ...
    'IsObjectiveDeterministic', true, ...
    'MaxObjectiveEvaluations', exploration_masses_iters, ...
    'UseParallel', parallel_var,...
    'MaxTime', inf, ...
    'NumSeedPoints', num_Seed_points_masses, ...
    'GPActiveSetSize', 300 ,...
    'PlotFcn', []);

%% SECOND PHASE: EXPLOITATION MASSES

results_masses = bayesopt(func_masses, opt_vars_masses, ...
    'Verbose', 0, ...
    'AcquisitionFunctionName', ...
    'expected-improvement', ...
    'IsObjectiveDeterministic', true, ...
    'MaxObjectiveEvaluations', exploitation_masses_iters+exploration_masses_iters, ...
    'UseParallel', parallel_var,...
    'MaxTime', inf, ...
    'InitialX', results_masses_explore.XTrace, ...
    'InitialObjective', results_masses_explore.ObjectiveTrace, ...
    'GPActiveSetSize', 300, ...
    'PlotFcn', []);

%% EXTRACT BEST VARIABLES

complete_results_masses_total{1,2} = {results_masses_explore};
complete_results_masses_total{2,2} = {results_masses};

estimated_masses=table2array(results_masses.bestPoint);
estimated_masses_complete =  zeros(1, 2*length(estimated_masses)-1); 
estimated_masses_complete(1:2:end) = estimated_masses;
...
N = length(results_masses.ObjectiveTrace);
idx_min = results_masses.IndexOfMinimumTrace(end);

figure
plot(1:N, results_masses.ObjectiveTrace, 'k*')
hold on;
min_ever = results_masses.ObjectiveMinimumTrace;
min_ever(1:exploration_masses_iters) = results_masses_explore.ObjectiveMinimumTrace;
...
set(h,'Interpreter', 'Latex');
grid('on');
xline(exploration_masses_iters, '--')
...
func =  @(x_vars)(obj_PID_panda_total_masses(x_vars,const,estimated_masses_complete));
initial_X = []; 
...
results_explore = bayesopt(func, opt_vars, ...
    ...
    'IsObjectiveDeterministic', true, ...
    'UseParallel', parallel_var,...
    'MaxObjectiveEvaluations', exploration_PID_iters, ...
    'MaxTime', inf, ...
    'NumSeedPoints', num_Seed_points_PID, ...
    'GPActiveSetSize', 300 ,...
    'PlotFcn', []);
...
results = bayesopt(func, opt_vars, ...
    ...
    'IsObjectiveDeterministic', true, ...
    'UseParallel', parallel_var,...
    'MaxObjectiveEvaluations', exploitation_PID_iters+exploration_PID_iters, ...
    'MaxTime', inf, ...
...
complete_results_masses_total{1,1} = {results_explore};
complete_results_masses_total{2,1} = {results};

%% Extract best variables

best_vars = results.bestPoint;
opt_cost=obj_PID_panda_total_masses(best_vars,const,estimated_masses_complete);
...
min_ever(1:exploration_PID_iters) = results_explore.ObjectiveMinimumTrace;
plot(min_ever, 'r', 'LineWidth', 2)
plot(idx_min, min_ever(idx_min), 'MarkerEdgeColor','black',...
    'MarkerFaceColor','gree', 'Marker', 'square', 'MarkerSize',10);
h=xlabel('Iteration index $i$ (-)');
...
set(h,'Interpreter', 'Latex');
grid('on');
xline(exploration_PID_iters, '--')
title('Objective function evolution plot')
legend('Current point', 'Current best point', 'Overall best point', 'Exploration vs exploitation');

%% OPTIMAL DYNAMIC SIMULATION

Robot_eval = panda_robot(estimated_masses_complete);
...
file_name = sprintf('complete_results_masses_total_%d_%d_%d_%d_%d_%d_%d_%d_%d_%d_%s_%s_aw.mat', ...
     exploration_masses_iters, exploitation_masses_iters, exploration_PID_iters, exploitation_PID_iters, w_pos_max, w_vel_max, w_pos_avg, w_vel_avg, w_jerk_mean,w_tau_max, sat_str,rampa_str); 
...
save(file_name, 'complete_results_masses_total');