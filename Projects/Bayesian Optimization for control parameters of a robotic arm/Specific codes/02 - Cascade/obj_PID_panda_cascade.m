function J_tot = obj_PID_panda_cascade(x_var, const)
...
while (jj<length(t) && exit_flag==false) 
    ...
    qerr(jj,dof_done)   = q_r(jj,dof_done) - q_msr(jj,dof_done);
    dqerr(jj,dof_done)  = dq_r(jj,dof_done) - dq_msr(jj,dof_done);
    ddqerr(jj,dof_done) = ddq_r(jj,dof_done) - ddq_msr(jj,dof_done);
...
J_tau_max = sum(max(abs(tau_PID(:,dof_done)+tau_comp(:,dof_done))).*tau_norm(dof_done)');
